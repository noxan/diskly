...
impl ScannerCore {
    ...
    #[cfg(unix)]
    pub(crate) fn get_file_size(&self, path: &Path, metadata: &fs::Metadata) -> u64 {
    ...
    #[cfg(not(unix))]
    pub(crate) fn get_file_size(&self, _path: &Path, metadata: &fs::Metadata) -> u64 {
        metadata.len()
    }
...
impl Scanner {
    ...
    pub fn scan_directory(&self, path: String) -> Result<(), String> {
        ...
        // Start scan
        match self.scan_recursive_with_events(&path_buf) {
            Ok(root) => {
    ...
    fn scan_recursive_with_events(&self, path: &Path) -> Result<DirNode, String> {
        ...
        let children: Vec<DirNode> = entries
            ...
            .filter_map(|entry| {
                ...
                self.scan_recursive_with_events(entry).ok()
            })
            .collect();
